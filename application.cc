// Initial Conditions Ln #245 Ln # 274
// Projection of initial condition Ln #434
...
void Step4<dim>::assemble_system ()
{
  ...
  VectorTools::create_point_source_vector(dof_handler,point,forcing_terms);
  for (; cell!=endc; ++cell)
    {
            ...
            for (unsigned int q_point=0; q_point<n_face_q_points; ++q_point)
              {
                const double neunmann_value
                 // = (exact_solution.gradient (fe_face_values.quadrature_point(q_point)) *
                  ...
                  = neunmannBC.value(fe_face_values.quadrature_point(q_point));
                for (unsigned int i=0; i<dofs_per_cell; ++i)
